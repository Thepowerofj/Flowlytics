...
function portsFor(blockType: string): { hasInput: boolean; hasOutput: boolean } {
...
function configColumns(cfg: Record<string, unknown>): string[] {
  ...
  if (Array.isArray(cfg._sourceColumns)) {
    return (cfg._sourceColumns as unknown[]).filter(
...
export function checkFlow(nodes: CheckNode[], edges: CheckEdge[]): FlowIssue[] {
  ...
  for (const n of nodes) {
    ...
    if (n.data.blockType === "transform.clean_map") {
      const cols = Array.isArray(cfg._sourceColumns)
        ? (cfg._sourceColumns as string[])
        : [];
    ...
    if (n.data.blockType === "output.structure") {
      ...
      const selected = Array.isArray(cfg.selectedColumns)
        ? (cfg.selectedColumns as string[])
        : [];
